...
training_data <- extract(rstack, train_polys_merge, df = TRUE) # 1,680,754 pixels
...
# see proportion coming from each class
table(training_datamerge$class)
...
train_ctrl <- trainControl(
  method = "cv",
  number = 5,
  index = index, # my custom folds by polyID
  verboseIter = TRUE,  # Print progress
  allowParallel = TRUE  # Enable caret parallelization
)

rf_grid <- expand.grid(
  mtry = c(3,4,5),  # Adjust based on predictors
  splitrule = "gini",
...
rstack_tile1 <- rast(mytile[grep("rstack_tile1", mytile)])
plot(rstack_tile1[[1]])
...
plot(classified_tile1)


##### Tile 2 #####
mytile <- list.files(output_folder, pattern = "*.tif$", full.names = TRUE)
rstack_tile2 <- rast(mytile[grep("rstack_tile2", mytile)])
plot(rstack_tile2[[1]])
...
plot(classified_tile2)


##### Tile 3 #####
mytile <- list.files(output_folder, pattern = "*.tif$", full.names = TRUE)
rstack_tile3 <- rast(mytile[grep("rstack_tile3", mytile)])
plot(rstack_tile3[[1]])
...
plot(classified_tile3)


##### Tile 4 #####
mytile <- list.files(output_folder, pattern = "*.tif$", full.names = TRUE)
rstack_tile4 <- rast(mytile[grep("rstack_tile4", mytile)])
plot(rstack_tile4[[1]])
...
# merge the tiles
classification2024 <- terra::mosaic(pred_tile1, 
                            pred_tile2,
                            pred_tile3,
                            pred_tile4)
plot(classification2024)
