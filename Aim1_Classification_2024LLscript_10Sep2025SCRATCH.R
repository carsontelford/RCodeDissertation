...
# rstack <- rstack[[!names(rstack) %in% c("B4","B7","B8","nirfocal","NDVI","redfocal")]]


# # see correlation between bands
# set.seed(123)
# sample_vals <- spatSample(rstack, size = 10000, method = "random", na.rm = TRUE)
...
xgb_grid <- expand.grid(
  nrounds = c(200,300),        # number of boosting rounds
  max_depth = c(3),
  eta = c(0.08),          # learning rate
  gamma = c(0),
