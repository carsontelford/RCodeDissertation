...
poly_folder <- "Aim 1/Shapefiles/Training Polygons/Training2022_2024_LONGLEAF/"
...
rstack <- rstack[[1:6]]
rstack <- aggregate(rstack, fact=7)
...
# # OLD CODE THAT DOESNT WORK ANYMORE BECAUSE OF THE GHOSTS
# rf_model <- ranger(
...
process_tile <- function(idx, tile_extents, xmins, xmaxs, ymins, ymaxs, rstack_file, model_file, output_folder) {
  ...
  tile_file <- file.path(output_folder, paste0("tilea_", k, "_classification2024.tif"))
...
classified_raster <- merge(
  mysprc,
  filename = file.path(output_folder, "classification2024TTT.tif"),
  overwrite = TRUE
)
