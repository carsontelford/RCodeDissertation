...
redfocalSD   <- focal(rstack$B4, w3, fun = sd,   na.policy = "omit", filename="redfocalSD.tif", overwrite=TRUE)
nirfocalSD   <- focal(rstack$B6, w3, fun = sd,   na.policy = "omit", filename="nirfocalSD.tif", overwrite=TRUE)
names(redfocalSD) <- c("redfocalSD")
names(nirfocalSD) <- c("nirfocalSD")
rstack <- c(rstack,redfocalSD, nirfocalSD)
names(rstack)

# see correlation between bands
set.seed(123)
sample_vals <- spatSample(rstack, size = 1e5, method = "random", na.rm = TRUE)
...
#### kml load ####
library(sf)
mykml <- st_read("C:/Users/carso/Downloads/DissertationAim1_2022_2024z.kml")
...
X <- train_set %>% dplyr::select(B11,B12,B2,B3,B4,B5,B6)
...
write.csv(cm_train$raw$wide, file.path(output_folder, "cm_train_confusionmatrix.csv"))
write.csv(cm_test$raw$wide, file.path(output_folder, "cm_test_confusionmatrix.csv"))
