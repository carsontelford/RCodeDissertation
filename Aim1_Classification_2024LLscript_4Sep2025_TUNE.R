...
class3 <- training_datamerge %>% filter(class == 3) %>% sample_frac(0.75)
class4 <- training_datamerge %>% filter(class == 4) %>% sample_frac(0.7)
...
xgb_grid <- expand.grid(
  nrounds = c(400, 500, 600,700),        # number of boosting rounds
  max_depth = c(3,4,5),
  eta = c(.06,.07,.08),           # learning rate
  gamma = c(0,2),
  colsample_bytree = c(.5, .6),
